...
class C_thanh_toan
{
	public function Hien_thi_thanh_toan()
	{
		//models
			...
			if(!isset($_SESSION['id_user'])) //Nếu chưa đăng nhập
			{
				$this->Thanh_toan_khach_vang_lai();	
			}
			else//Nếu đã đăng nhập
			{
				$this->Thanh_toan_Thanh_vien();
			}
	...
	public function Thanh_toan_khach_vang_lai()
	{
		if(isset($_POST['btn_thanh_toan']))
			{
				...
				{
					//nối chuối địa chỉ
					include_once("models/m_tinh_thanh.php");
					$m_tinh_thanh=new M_tinh_thanh();
	...
	public function Thanh_toan_Thanh_vien()
	{
		if(isset($_POST['btn_thanh_toan']))
		{
			$ho_ten=$_POST['ho_ten_dh'];
			...
			$dia_chi=$_POST['dia_chi_dh'];
			$ghi_chu=$_POST['ghi_chu_dh'];
			$thanh_vien=1;// 0:chưa là thành vien( chưa tài khoản). 1:là thành viên(có tài khoản)
			$trang_thai=0; // Chưa thanh toán
			$hinh_thuc=$_POST['hinh_thuc_dh'];
